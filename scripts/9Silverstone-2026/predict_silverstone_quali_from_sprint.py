...
def predict_quali_from_practice(practice_features):
    ...
    safe_weight("best_FP1_lap_seconds", 0.40)
    safe_weight("best_SQ_lap_seconds", 0.60)
...
sq_display = session_displays["SQ"]

# Dictionary of best laps per session
fp1_best = best_laps_by_session["FP1"]
sq_best = best_laps_by_session["SQ"]

# Removes the Team column from FP1 and SQ to avoid duplicate columns when merging
fp1_best = fp1_best.drop(columns=["Team"])
sq_best = sq_best.drop(columns=["Team"])

# Filter to drivers who participated in at least one of SQ as well as FP1
valid_drivers = set(sq_best["Driver"])
...
display_output = quali_prediction[['predicted_quali_position', 'Driver', 'best_FP1_lap', 'best_FP1_theoretical_lap',
                                   'best_SQ_lap', 'best_SQ_theoretical_lap', 'prediction_score']]
